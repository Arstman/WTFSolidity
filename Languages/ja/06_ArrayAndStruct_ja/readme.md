...
- 配列リテラルは１つまたは複数の式の形をした配列で、すぐに変数へ代入されるわけではありません。例えば、`[uint(1),2,3]`のように記述します。各要素の型は、最初の要素と周囲の文脈から推論されます。値に型が指定されていない場合は、デフォルトで最小の型が使用されます。例えば、`[1,2,3]`では`uint8`が、`[uint(1),2,3]`では`uint`が使用されます。
